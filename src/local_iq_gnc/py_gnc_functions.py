...
class gnc_api:
    ...
    def disarm(self):
        ...
        Returns:
                0 (int): Disarming successful.
                -1 (int): Disarming unsuccessful.
        """
        self.set_destination(0, 0, 0, 0)
        ...
        rospy.loginfo(CBLUE2 + "Disarming Drone" + CEND)

        disarm_request = CommandBoolRequest(False)

        while not rospy.is_shutdown() and self.current_state_g.armed:
            rospy.sleep(0.1)
            response = self.arming_client(disarm_request)
            self.local_pos_pub.publish(self.waypoint_g)
        else:
            if response.success:
                rospy.loginfo(CGREEN2 + "Disarming successful" + CEND)
                return 0
            else:
                rospy.logerr(CRED2 + "Disarming failed" + CEND)
                return -1
